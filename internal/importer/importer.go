...
func Import(ctx context.Context, opt *Options) error {
	...
	if opt.Pool == nil || opt.Recorder == nil || opt.Config == nil {
		log.Debug().Interface("options", opt).Msg("missing required fields")
		return errors.New("options missing required fields")
	}

	opt.Recorder.One(ctx, "import_run")
	...
	opt.Recorder.WithLabels(ctx, "vulns_found", len(vuls), map[string]string{
		"image":  opt.ImageURI,
